...
def do_sample( top_chunk_scores, bottom_chunk_scores, wpg_data_file ):
    ...
    # TO DO: filter based on isbns in top and bottom score dictionaries
...
def filter_chunks( chunk_tuples, chunk_scores, threshold, bigger_than=True ):
    if bigger_than:
        ...
        for isbn, chunks in chunk_tuples:
            print("isbn:", isbn)
            print("chunks:", len(chunks))
            print("chunk scores:", chunk_scores[isbn])
            for chunk, score in zip(chunks, chunk_scores[isbn]):
...
def make_dist( chunks, doc_freq ):
    return Counter([term for chunk in chunks for term in tokenize_chunk( chunk ) if doc_freq[term] > 1])

def get_most_frequent_terms( isbn_data, chunk_scores ):
    text_chunks = get_text_chunks( isbn_data )
    doc_freq = compute_doc_freq( text_chunks )
    chunks = filter_chunks( text_chunks, chunk_scores, 0.8 )
    fdist = make_dist( chunks, doc_freq )
    top_terms = [term for term, freq in fdist.most_common(100)]
    return top_terms, fdist
...
if __name__ == "__main__":
    ...
    bottom_text_chunks = get_text_chunks( bottom_isbn_data )
    print("number of isbns:", len(bottom_text_chunks))
    bottom_doc_freq = compute_doc_freq( bottom_text_chunks )
    print("number of bottom terms:", len(bottom_doc_freq.items()))
    bottom_chunks = filter_chunks( bottom_text_chunks, chunk_scores, 0.5, bigger_than=False )
    print("number of chunks:", len(bottom_chunks))
    bottom_fdist = make_dist( bottom_chunks, bottom_doc_freq )
    print(bottom_fdist.most_common(100))
    bottom_terms = [term for term, freq in bottom_fdist.most_common(100)]
    #top_terms, top_fdist = get_most_frequent_terms( top_isbn_data, chunk_scores )
    #top_only = [term for term in top_terms if term not in bottom_terms]
    #bottom_only = [term for term in bottom_terms if term not in top_terms]
    #for term in top_only:
    #    print("top only:", term, top_fdist[term], top_doc_freq[term])
    #for term in bottom_only:
    #    print("bottom only:", term, bottom_dist[term], bottom_doc_freq[term])
